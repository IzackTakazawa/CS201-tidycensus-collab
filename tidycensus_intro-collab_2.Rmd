...
oahu_one_race_hawaiian_other <- get_acs(
  geography = "tract",
  variables = c(native_hawaiian_other = "DP05_0052P"),
  state = "HI",
  county = "Honolulu",
  geometry = TRUE,
  output = "wide",
...
oahu_hawaiian_other_map <- ggplot(oahu_one_race_hawaiian_other) +
  geom_sf_interactive(
    aes(
      fill = native_hawaiian_otherE,
      tooltip = paste(NAME, ": ", native_hawaiian_otherE, "%"),
      data_id = NAME
    )
  ) +
...
girafe(ggobj = oahu_hawaiian_other_map, width = 700, height = 400)
...
oahu_hawaiian_other_2009_2019 <- map_dfr(years, ~{
  get_acs(
    geography = "county",
    variables = c(native_hawaiian_other = "DP05_0052P"),
    state = "HI",
...
```{r, warning=FALSE}
ggplot(oahu_hawaiian_other_2009_2019, aes(x = year, y = estimate, group = 1)) + 
  geom_line() + 
  geom_point()
```
...
ggplot(oahu_hawaiian_other_2009_2019, aes(x = year, y = estimate, group = 1)) + 
  ...
  labs(title = "Percentage of Families Only Native Hawaiian or Other Pacific Islander: Honolulu County",
       x = "Year",
       y = "%",
...
```{r, message=FALSE}
oahu_native_hawaiian_other_one_race <- get_acs(
  geography = "tract",
  variables = c(native_hawaiian_other = "DP05_0052P",
                percent_of_japanese_one_race = "DP05_0048P"),
  state = "HI",
  county = "Honolulu",
  #geometry = TRUE,
  output = "wide",
...
ggplot(oahu_native_hawaiian_other_one_race, aes(x = native_hawaiian_otherE,
                                             y = percent_of_japanese_one_raceE)) +
  geom_point() +
  geom_smooth() +
  stat_cor() +
  theme_minimal()
...
From our data, we found that there are more families of only Japanese ethnicity than families of only Hawaiian or Pacific Islander decent. And the percentage of families of only Native Hawaiian or Pacific Islander significantly grew after 2016.
